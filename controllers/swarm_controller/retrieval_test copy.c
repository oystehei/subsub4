...
#define REVERSE_LIMIT_RETRIEVAL 40
#define FORWARD_LIMIT_RETRIEVAL 30
#define TURN_LIMIT_RETRIEVAL 10
...
int reverse_counter_retrieval = 0;
int forward_counter_retrievel = 0;
int turn_counter_retrieval = 0;
int turn_left_retrieval = FALSE;
...
void swarm_retrieval(double IR_sensor_value[8], double ps_sensor_value[8], int IR_threshold)
{
	...
	if(converge_check == TRUE){
	...
	else if(move_away == TRUE){
		if(reverse_counter_retrieval < REVERSE_LIMIT_RETRIEVAL){
			left_wheel_speed = -1000;
			right_wheel_speed = -1000;
			reverse_counter_retrieval = reverse_counter_retrieval + 1;
		}
		else if(turn_counter_retrieval != TURN_LIMIT_RETRIEVAL){
			turn_counter_retrieval = turn_counter_retrieval +1;
			if(turn_left_retrieval == NEUTRAL){
				// Roll a dice, left or right?
				double ran = rand()/((double)(RAND_MAX)+1);
				if (ran > 0.5)
					turn_left_retrieval = FALSE;
				else
					turn_left_retrieval = TRUE;
			}

			if(turn_left_retrieval){ // Turn left
		...
		else if(forward_counter_retrievel < FORWARD_LIMIT_RETRIEVAL){
			left_wheel_speed = 1000;
			right_wheel_speed = 1000;
			forward_counter_retrievel = forward_counter_retrievel + 1;
		}
		else{
			move_away = FALSE;
			reverse_counter_retrieval = 0;
			forward_counter_retrievel = 0;
			turn_counter_retrieval = 0;
			turn_left_retrieval = NEUTRAL;
		}
	}
	else{
		if(push){
